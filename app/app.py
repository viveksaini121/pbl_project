...
1) Upload a network-flow CSV file OR provide a local file path
...
st.write(
    "Upload a network CSV file (or use a local file path for very large files), run attack prediction, and view SHAP explanation."
)
...
input_mode = st.radio(
    "Choose input method",
    ["Upload CSV", "Use local CSV path"],
    help="Use local path when files are larger than browser upload limits.",
)
...
if input_mode == "Upload CSV":
    ...
    local_csv_path = st.text_input(
        ...
        help="Path on the machine where Streamlit is running (example: /Users/you/.../02-14-2018.csv)",
    )
    if not local_csv_path:
        st.info("Please enter a local CSV path to continue.")
        st.stop()

    csv_path = Path(local_csv_path).expanduser()
    if not csv_path.exists():
        st.error(f"File not found: {csv_path}")
        st.stop()

    input_df = pd.read_csv(csv_path)
    st.caption(f"Loaded from local path: {csv_path}")
